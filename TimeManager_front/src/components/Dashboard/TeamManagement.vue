...
const updateTeamUserId = (id, bool) => {
  TeamUserId.value = id
  isUserSelected.value = bool
  fetchWorkingTimes()
}
...
onMounted(async () => {
  TeamUserId.value = localStorage.getItem("id")
  ...
  <div>
    ...
    <div class="grid grid-cols-3 gap-6">
      ...
      <div class="row-span-2 flex flex-col items-start bg-light_bg shadow rounded-lg p-4">
        ...
        <div class="my-2 w-full" @click="logId()">
          <p class="text-xl font-semibold">My Teams</p>
          <TeamsTable @update-team-user-id="updateTeamUserId" />
        </div>
      </div>
